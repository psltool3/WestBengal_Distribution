...
if($numrows>0){
	...
	if($status==0){
		...
		writeLog("District User ->" ." Warehouse Active -> ". $_SESSION['district_user'] . "| " . $warehousename);
		mysqli_query($con,$query);
	}
	else{
		$query = "UPDATE warehouse SET active='0' WHERE uniqueid='$id'";
		writeLog("District User ->" ." Warehouse InActive -> ". $_SESSION['district_user'] . "| " . $warehousename);
		mysqli_query($con,$query);
	}
}
