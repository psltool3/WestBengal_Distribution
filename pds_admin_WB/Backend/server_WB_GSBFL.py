...
def extract_db():
    if request.method == 'POST':
        ...
        if connection.is_connected():
            cursor = connection.cursor()
            query = "SELECT * FROM depot WHERE active='1'"
            cursor.execute(query)
            user = cursor.fetchall()
        ...
        if connection.is_connected():
            ...
            if user:
                for row in user:
                    temp = {'State Name':'','FPS_District': row[0], 'FPS_Name': row[1], 'FPS_ID': row[2], 'Motorable/Non-Motorable': row[3], 'FPS_Lat': row[4], 'FPS_Long': row[5], 'Allocation_Atta': float(row[6])*int(applicableCount),'Allocation_Wheat': float(row[7])*int(applicableCount), 'Allocation_Rice': float(row[9])*int(applicableCount),'Allocation_FRice': float(row[10])*int(applicableCount)}
        ...
        fps = fps.loc[:,["State Name","FPS_District",'FPS_Name',"FPS_ID","Motorable/Non-Motorable",'FPS_Lat',"FPS_Long","Allocation_Wheat","Allocation_Rice","Allocation_FRice","Allocation_Atta"]]
    ...
    if request.method == 'POST':
        try:
            ...
            if connection.is_connected():
                ...
                has_atta = 'Atta' in wh['commodity'].unique()
                has_wheat = 'Wheat' in wh['commodity'].unique()
...
def fci_data():
    try:
        ...
        total_demand_atta = float(fps['Allocation_Atta'].sum())
        ...
        result = {'Warehouse_No': warehouse_no, 'FPS_No': fps_no, 'Total_Demand': total_demand,'Total_Demand_Rice': total_demand_rice, 'Total_Supply': total_supply1, 'District_Count': districts_no,'Total_Demand_FRice': total_demand_frice,'Mill_No':mill_no, 'Total_Supply2': total_supply2,'Total_Processing':total_processing,'Wholesale_No': wholesale_no,'Total_Wholesalecapacity':total_wholesalecapacity,'Total_Demand_Atta': total_demand_atta,}
...
def graph_dataleg1():
    try:
        ...
        fci = pd.read_excel(usn, sheet_name='A.2 FCI', index_col=None)
        mill = pd.read_excel(usn, sheet_name='A.3 MILL', index_col=None)
        warehouse = pd.read_excel(usn, sheet_name='A.1 Warehouse', index_col=None)
        ...
        District_Capacity1 = {}
        for i in range(len(warehouse["WH_District"])):
            District_Name = warehouse["WH_District"][i]
            if District_Name not in District_Capacity1:
                District_Capacity1[District_Name] = float(warehouse["Raw_Rice"][i])
            else:
                District_Capacity1[District_Name] += float(warehouse["Raw_Rice"][i])        
        ...
        for i in range(len(warehouse["WH_District"])):
            ...
            if District_Name not in District_Capacity2:
                District_Capacity2[District_Name] = float(warehouse["Parboiled_Rice"][i])
            else:
                District_Capacity2[District_Name] += float(warehouse["Parboiled_Rice"][i])
        ...
        combined_data = {'District_Demand': District_Demand, 'District_Capacity': District_Capacity, 'District_Name': District_Name_1,'District_Demand_RR':District_Demand_RR,'District_Demand_PR':District_Demand_PR, 'District_Millprocessing':District_Millprocessing,'District_Capacity1': District_Capacity1,'District_Capacity2': District_Capacity2,}
...
def save_to_database(month, year, applicable):
    ...
    if connection.is_connected():
        ...
        create_fps_query = ("CREATE TABLE " + fps_table + " (district VARCHAR(100) NOT NULL, name VARCHAR(100) NOT NULL, id VARCHAR(100) NOT NULL, type VARCHAR(100) NOT NULL, latitude VARCHAR(100) NOT NULL, longitude VARCHAR(100) NOT NULL, demand VARCHAR(100) NOT NULL, demand_wheat VARCHAR(100) NOT NULL,uniqueid VARCHAR(100) NOT NULL, demand_rice VARCHAR(100) NOT NULL, demand_frice VARCHAR(100) NOT NULL, active VARCHAR(10) NOT NULL DEFAULT '1')")
...
def save_to_database_leg1(month, year, applicable):
    ...
    if connection.is_connected():
        ...
        if(check_year_month_exists_leg1(connection, month, year)):
            ...
            sql = "UPDATE optimised_table_leg1 set applicable='" + applicable + "', last_updated='" + formatted_datetime + "' , cost = '""' WHERE id='" + existingid + "'"; 
...
def processFile():
    ...
    write_log("User -> " + " Optimization Start Leg2 Requested JSON -> " + str(json_data))
    ...
    #    Variables for Wheat from IG TO FPS
        ...
        for i in range(len(FPS["FPS_District"])):
            ...
            if District_Name_FPS not in District_Demand_RR:
                ...
                District_Demand_RR[District_Name_FPS] +=  float(FPS["Allocation_Rice"][i]) + float(FPS["Allocation_FRice"][i])+float(FPS["Allocation_Wheat"][i])+ float(FPS["Allocation_Atta"][i]) 
        ...
        FPS['Demand'] = FPS['Allocation_Wheat'] + FPS['Allocation_Rice'] + FPS['Allocation_FRice']+ FPS['Allocation_Atta']
       ...
       # Calling CBC_CMB Solver
        ...
        df4111 = pd.merge(df31, FCI, on='WH_ID', how='inner')
        df4111 = df4111[[
            'WH_ID',
            'WH_Name',
            'WH_District',
        ...
        df4111 = pd.merge(df4111, FPS, on='FPS_ID', how='inner')
        df51111 = df4111[[
            'WH_ID',
            'WH_Name',
            'WH_District',
        ...
        df51111.insert(0, 'Scenario', 'Optimized')
        df51111.insert(1, 'From', 'Wholesale')
        df51111.insert(2, 'From_State', 'West Bengal')
        df51111.insert(7, 'To', 'FPS')
        df51111.insert(8, 'To_State', 'West Bengal')
        df51111.insert(9, 'commodity', 'Atta')
  
        df51111.rename(columns={
            'WH_ID': 'From_ID',
            'WH_Name': 'From_Name',
            'WH_Lat': 'From_Lat',
            'WH_Long': 'From_Long',
        ...
        df51111.rename(columns={
            ...
            'FPS_Lat': 'To_Lat',
            'FPS_Long': 'To_Long',
            'Allocation_Atta': 'quantity',
            
            }, inplace=True)
        df51111.rename(columns={'WH_District': 'From_District',
                   'FPS_District': 'To_District'}, inplace=True)
        df51111 = df51111.loc[:, [
            'Scenario',
            'From',
        ...
        df_combined = pd.concat([df51, df511,df5111,df51111])
...
# -----------------------------------------
    ...
    #    Variables for Wheat from IG TO FPS
       ...
       # Calling CBC_CMB Solver
        ...
        df51111.insert(7, 'To', 'FPS')
        df51111.insert(8, 'To_State', 'West Bengal')
        df51111.insert(9, 'commodity', 'Wheat')
...
def processFile_leg1():
    ...
    stop_process = False
    json_data = request.form
    write_log("User -> " + " OptimizationLeg1 Start Leg2 Requested JSON -> " + str(json_data))
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
        ...
        for i in range(len(MILL['WH_ID'])):                  
            model += ((lpSum(Allocation6[j][i] for j in range(len(FCI['WH_ID'
                           ])))) ==1.06 * (lpSum(Allocation7[i][k] for k in range(len(WH['SW_ID'
                           ])))))
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
        ...
        USN = pd.ExcelFile('Backend//Data_2.xlsx')
        DEPOT = pd.read_excel(USN, sheet_name='A.2 FCI', index_col=None)
        WH = pd.read_excel(USN, sheet_name='A.2 WHOLESALE', index_col=None)
        ...
        df5['Distance'] = df5['Tagging'].map(Distance_BG_BG)
        df5.fillna('shallu', inplace=True)
        df5.to_excel('Backend//Result_Sheet12_Leg111.xlsx', sheet_name='Warehouse_FPS', index=False)        


        
# ----------------------------------------------------------------------------------------------------------------------------------------------
        Result_Sheet1 = pd.ExcelFile("Backend//Result_Sheet12_Leg111.xlsx")
        df6 = pd.read_excel(Result_Sheet1, sheet_name="Warehouse_FPS")
        Result_Sheet1.close()
        ...
        # Save to Excel
        #print('result sheet generated')
        df10.to_excel('Backend//Result_Sheet_leg4.xlsx', sheet_name='Warehouse_FPS', index=False)
# ---------------------------------------------------------------------------------------------------------------------------------------------
        ...
        df_leg4 = pd.read_excel('Backend/Result_Sheet_leg4.xlsx', sheet_name='Warehouse_FPS')

        # Append (row-wise, same columns)
        df_result = pd.concat([df_leg2, df_leg3,df_leg4], ignore_index=True)
