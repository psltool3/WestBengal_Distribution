							...
							<div class="row">
								...
								<div class="col-md-3 mb-3">
									<div class="card h-100"
										...
										<div style="font-size:14px">Total Parboiled Rice (Qtl)</div>
									</div>
								</div>
								<div class="col-md-3 mb-3">
									<div class="card h-100"
								...
								<div class="col-md-3 mb-3">
									<div class="card h-100"
										style="background-color:#00CC00; color:white; padding:12px; font-weight: bold;">
										<div style="font-size:20px" id="total_mill"></div>
								...
								<div class="col-md-3 mb-3">
									<div class="card h-100"
										...
										<div style="font-size:20px" id="total_processing"></div>
										<div style="font-size:14px">Total Mill Processing (Qtl)</div>
									</div>
								</div>
								<div class="col-md-3 mb-3">
									<div class="card h-100"
					...
					<div
						...
						<div class="card">
							<div class="row">
								...
								<center style="margin-top:20px">
									<h4><b id="totalmillprocessing"></b></h4>
								</center>
								<center style="margin-top:20px">
									<h4><b id="totalFciDemand"></b></h4>
								</center>
								<center style="margin-top:20px">
									<h4><b id="totalwholesale1"></b></h4>
								</center>
								<center style="margin-top:20px">
...
<script>
		...
		$.ajax({
			...
			success: function(result){
				...
				try{
					...
					document.getElementById("totalwholesale1").innerHTML = "";
					document.getElementById("totalFciSupply").innerHTML = "";
					document.getElementById("totalFciSupply1").innerHTML = "";
					document.getElementById("totalmillprocessing").innerHTML = "";
						...
						.then(data => {
							...
							fetch(pythonUrl + 'getfcidata', {
								...
								.then(data => {
									...
									document.getElementById("total_mill").innerHTML = data["Mill_No"].toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
									document.getElementById("total_processing").innerHTML = data["Total_Processing"].toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
									
									document.getElementById("total_wholesalecapacity").innerHTML = data["Total_Wholesalecapacity"].toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
							        ...
							        document.getElementById("total_supply2").innerHTML = data["Total_Supply2"].toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
	...
	function handleDistrictCheckboxChange() {
		...
		if (checkbox.checked) {
			...
			if(type=="intra")
			{
				if (district_names1["District_Name_All1"].length > 0) {
					var concatenatedNames1 = district_names1["District_Name_All1"].join(', ');
					document.getElementById("resultdistrict1").innerHTML = "Intra district movement is infeasble for District- " + concatenatedNames1;
					document.getElementById("resultdistrict1").style.color = "#ADFF2F"; 
				} else {
					document.getElementById("resultdistrict1").innerHTML = "Intra scenario in every district is feasible ";
	...
	if (checkbox.checked) {
		...
		fetch(pythonUrl + 'getGraphData', {
			...
			.then(data => {
				...
				document.getElementById("totalmillprocessing").innerHTML = "<span style='color: white; font-size: 14px;'>" + "Total Mill Processing: " + totalprocessing.toFixed(2) + " (Qtl)Demand_Atta: " + totalDemandWheat.toFixed(2) +
				"</span>";
				document.getElementById("totalFciDemand").innerHTML = "<span style='color: white; font-size: 14px;'>" + "Total Demand (RR+PR+Atta): " + totalDemand.toFixed(2) + " (Qtl)</span>";
				
				document.getElementById("totalwholesale1").innerHTML = "<span style='color: white; font-size: 14px;'>" + "Total Whole Sale Capacity: " + totalwholesale.toFixed(2) + " (Qtl)</span>";
				
				document.getElementById("totalFciSupply").innerHTML = 
				"<span style='color: white; font-size: 14px;'>" +
				"Total RR: " + totalCapacity.toFixed(2) + " (Qtl) Demand_RR: " + totalDemandRice.toFixed(2) +
				"</span>";

			    document.getElementById("totalFciSupply1").innerHTML = 
				"<span style='color: white; font-size: 14px;'>" +
				"Total PR: " + totalCapacity1.toFixed(2) + " (Qtl) Demand_PR: " + totalDemandFRice.toFixed(2) +
				"</span>";
				...
				if (totalCapacity >= -1 && totalDemandRice >= -1 && totalCapacity1 >= -1 && totalDemandFRice >= -1 && totalprocessing >= -1 && totalDemandWheat >= -1) {
					...
					var processing = unionArray.map(district => data.Mill_Processing[district]);
					...
					var newData = {
						...
						datasets: [
							...
							{
								label: 'Demand Raw Rice',
								backgroundColor: '#c64a44',
								data: demands_rice
							},
							{
								label: 'Allocation Raw Rice',
								backgroundColor: '#c64a44',
								data: capacities
							},
							{
								label: 'Demand Parboiledrice',
