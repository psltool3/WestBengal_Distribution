...
$mapData = [
    ...
    "Latitude" => "latitude",
    "Longitude" => "longitude",
     "Allocation of Raw_Rice" => "demand",
	"Allocation of Parboiled_Rice" => "demand_rice",
			...
			while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
				if($i>0){
					...
					if(!isStringNumber($column[$demand])){
						echo "Error : Check Capacity of Depot Value: ".$column[$demand];
						echo "</br>";
						$redirect = 0;
					}	
					if(!isStringNumber($column[$demand_rice])){
						echo "Error : Check Processing Capacity of Depot Value: ".$column[$demand_rice];
						echo "</br>";
						$redirect = 0;
					}	
					...
					$query_check = $Depot->checkEdit($Depot);
					$query_result = mysqli_query($con, $query_check);
					$numrows = mysqli_num_rows($query_result);
					if($numrows==0){
						echo "Error : Error in loading data as Depot id doesn't exist : ".$column[$id];
						echo "</br>";
						$redirect = 0;
					}
					writeLog("User ->" ." Depot Edit -> ". $_SESSION['user'] . "| " . $Depot->getName());
					$query_update = $Depot->updateEdit($Depot);
		...
		if ($_FILES["file"]["size"] > 0) {
			...
			while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
				if($i>0){
					...
					$uniqueid = uniqid("Depot_",);
					$Depot->setUniqueid(substr($uniqueid,0,15));
					$Depot->setDistrict($column[$district]);
					$Depot->setLatitude($column[$latitude]);
					$Depot->setLongitude($column[$longitude]);
					$Depot->setName($column[$name]);
					$Depot->setId($column[$id]);
					...
					$numrows = mysqli_num_rows($query_result);
					if($numrows==0){
						echo "Error : in loading data as Depot id doesn't exist : ".$column[$id];
						echo "</br>";
						$redirect = 0;
					}
					$query_update = $Depot->updateEdit($Depot);
