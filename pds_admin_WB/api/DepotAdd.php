...
if(!isStringNumber($_POST["demand"])){
	echo "Error : Check Capacity of Depot Value";
	exit();
}


if(!isStringNumber($_POST["demand_rice"])){
	echo "Check Processing capacity of Depot Value";
	exit();
}
...
$Depot->setDistrict(ucwords(strtolower($district)));
$Depot->setLatitude($latitude);
$Depot->setLongitude($longitude);
$Depot->setName($name);
$Depot->setId($id);
$Depot->setType($type);
$Depot->setDemand($demand);
$Depot->setDemandrice($demand_rice);
$Depot->setActive("1");
...
if($numrows_insert==0){
	$query = $Depot->insert($Depot);
	mysqli_query($con, $query);
	mysqli_close($con);
	$filteredPost = $_POST;
	unset($filteredPost['username'], $filteredPost['password']);
	writeLog("User ->" ." Depot added ->". $_SESSION['user'] . "| Requested JSON -> " . json_encode($filteredPost));
	echo "<script>window.location.href = '../Depot.php';</script>";
}
else{
	echo "Error : Error in Insertion as Depot id already exist";
}
