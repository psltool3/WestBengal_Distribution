...
$query_check = $Depot->checkInsert($Depot);
...
if($numrows!=0){
	...
	if($uniqueid!=$uniqueid_check){
		echo "Error : in updating data as Depot id already exist ID: ".$id;
		echo "</br>";
		exit();
	}
}

$query = $Depot->update($Depot);
...
writeLog("User ->" ." Depot Edit->". $_SESSION['user'] . "| Requested JSON -> " . json_encode($filteredPost));
